def test_seeded_meals_exist(client):
    ...
    # seeding should add 3 meals on a fresh DB
    assert len(meals) == 3
...
def test_get_meal_by_id(client):
    # create first
    created = client.post("/api/v1/meals", json={"name": "Find Me", "price": 9.99}).json()
    meal_id = created["id"]
...
def test_update_meal_partial(client):
    created = client.post("/api/v1/meals", json={"name": "Old", "price": 1.0}).json()
    meal_id = created["id"]

    r = client.put(f"/api/v1/meals/{meal_id}", json={"price": 2.5})
    assert r.status_code == 200
...
def test_delete_meal(client):
    created = client.post("/api/v1/meals", json={"name": "Delete Me", "price": 3.33}).json()
    meal_id = created["id"]
